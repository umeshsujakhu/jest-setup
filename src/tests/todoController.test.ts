...
describe('POST /todo', () => {
  ...
  it('should show validation error if completed field is not boolean', async () => {
    ...
    expect(res.status).toEqual(400);
    expect(res.body.success).toBe(false);
    expect(res.body.errors).toEqual(expect.arrayContaining([expect.objectContaining({ path: 'completed' })]));
  });
...
describe('PUT /todo/:id', () => {
  ...
  it('should update an existing todo', async () => {
    const createTodoRes = await request(app).post('/api/v1/todo').send({ title: 'Test Todo', completed: false });

    const createdTodoId = createTodoRes.body.data._id;

    const updateTodoRes = await request(app).put(`/api/v1/todo/${createdTodoId}`).send({ title: 'Updated Test Todo', completed: true });
  ...
  it('should return 404 when updating a non-existing todo', async () => {
    ...
    const updateTodoRes = await request(app).put(`/api/v1/todo/${nonExistingTodoId}`).send({ title: 'Test Todo', completed: true });
...
describe('DELETE /todo/:id', () => {
  it('should delete an existing todo', async () => {
    ...
    const deleteTodoRes = await request(app).delete(`/api/v1/todo/${createdTodoId}`);
  ...
  it('should return 404 when deleting a non-existing todo', async () => {
    ...
    const deleteTodoRes = await request(app).delete(`/api/v1/todo/${nonExistingTodoId}`);
...
describe('PATCH /todo/:id/status', () => {
  it('should return 404 when updating a non-existing todo status', async () => {
    ...
    const updateTodoRes = await request(app).put(`/api/v1/todo/${nonExistingTodoId}/status`).send({ completed: true });
  ...
  it('should show validation error if completed field is not boolean', async () => {
    ...
    const res = await request(app).patch(`/api/v1/todo/${todoId}/status`).send({ completed: 'random string' });
  ...
  it('should show required validation error when completed field is missing', async () => {
    ...
    const res = await request(app).patch(`/api/v1/todo/${todoId}/status`).send({});
  ...
  it('should update the status of an existing todo', async () => {
    ...
    const updateStatusRes = await request(app).patch(`/api/v1/todo/${createdTodoId}/status`).send({ completed: true });
